...
/**
 * Berechnet den aktuellen Drehwinkel im Bereich 0.0 bis 360.0 Grad.
 * Wandelt Schritte (auch negative) in eine positive Kreisposition um.
 * * @param totalSteps Die absolute Anzahl der Schritte seit Start (von der FSM).
 * @return Winkel in Grad (0.0 <= w < 360.0).
 */
double calcRotationalAngle(long totalSteps);

/**
 * Berechnet die Winkelgeschwindigkeit in Grad pro Sekunde.
