...
double calcRotationalAngle(long totalSteps) {
    ...
    long stepsInCircle = totalSteps % (long)STEPS_PER_REVOLUTION;

    // 2. Negative Werte korrigieren
    // Beispiel: -300 Schritte (=-90°) sollen als 270° (Position) angezeigt werden.
    if (stepsInCircle < 0) {
        stepsInCircle += (long)STEPS_PER_REVOLUTION;
    }

    // 3. In Grad umrechnen: (Schritte / 1200) * 360
    double angle = ((double)stepsInCircle / STEPS_PER_REVOLUTION) * DEGREES_PER_CIRCLE;
